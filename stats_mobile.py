...
f = open('./project_campaign_mobile/project_campaign_mobile.json', 'r')
data = json.load(f)
...
for i in data['results']:
    id = data['results'][i]['meta']['id']
    file = open('./project_campaign_mobile/data/' + id + '/stdout', 'r')
    ...
    count_file = open('./project_campaign_mobile/data/' +
                      id + '/count.txt', 'r')
...
for i in range(len(thrs)):
    axes[i // 3][i % 3].set_title(str(maxranges[i // len(maxranges)]
                                      ) + ' - ' + str(nstations[i % len(nstations)]))
