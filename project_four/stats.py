...
df_thr = df.groupby(['rtscts', 'hidden', 'datarate', 'packetsize']
                    ).thr.agg(['mean', 'std']).reset_index()
df_rtr = df.groupby(['rtscts', 'hidden', 'datarate', 'packetsize']
                    ).rtr.agg(['mean', 'std']).reset_index()

thrs = []
rtrs = []
for s in packetsizes:
    means_t = df_thr.loc[(df_thr['packetsize'] == s) & (df_thr['hidden'] == True) & (
        df_thr['rtscts'] == True)].sort_values(by=['datarate'], ascending=True)['mean']
    stds_t = df_thr.loc[(df_thr['packetsize'] == s) & (df_thr['hidden'] == True) & (
        df_thr['rtscts'] == True)].sort_values(by=['datarate'], ascending=True)['std']
    means_f = df_thr.loc[(df_thr['packetsize'] == s) & (df_thr['hidden'] == True) & (
        df_thr['rtscts'] == False)].sort_values(by=['datarate'], ascending=True)['mean']
    stds_f = df_thr.loc[(df_thr['packetsize'] == s) & (df_thr['hidden'] == True) & (
    ...
    means_t = df_rtr.loc[(df_rtr['packetsize'] == s) & (df_rtr['hidden'] == True) & (
        df_rtr['rtscts'] == True)].sort_values(by=['datarate'], ascending=True)['mean']
    stds_t = df_rtr.loc[(df_rtr['packetsize'] == s) & (df_rtr['hidden'] == True) & (
        df_rtr['rtscts'] == True)].sort_values(by=['datarate'], ascending=True)['std']
    means_f = df_rtr.loc[(df_rtr['packetsize'] == s) & (df_rtr['hidden'] == True) & (
        df_rtr['rtscts'] == False)].sort_values(by=['datarate'], ascending=True)['mean']
    stds_f = df_rtr.loc[(df_rtr['packetsize'] == s) & (df_rtr['hidden'] == True) & (
        df_rtr['rtscts'] == False)].sort_values(by=['datarate'], ascending=True)['std']
    rtrs.append({'means_t': list(means_t), 'stds_t': list(stds_t),
                 'means_f': list(means_f), 'stds_f': list(stds_f)})

    means_t = df_thr.loc[(df_thr['packetsize'] == s) & (df_thr['hidden'] == False) & (
        df_thr['rtscts'] == True)].sort_values(by=['datarate'], ascending=True)['mean']
    stds_t = df_thr.loc[(df_thr['packetsize'] == s) & (df_thr['hidden'] == False) & (
        df_thr['rtscts'] == True)].sort_values(by=['datarate'], ascending=True)['std']
    means_f = df_thr.loc[(df_thr['packetsize'] == s) & (df_thr['hidden'] == False) & (
        df_thr['rtscts'] == False)].sort_values(by=['datarate'], ascending=True)['mean']
    stds_f = df_thr.loc[(df_thr['packetsize'] == s) & (df_thr['hidden'] == False) & (
        df_thr['rtscts'] == False)].sort_values(by=['datarate'], ascending=True)['std']
    thrs.append({'means_t': list(means_t), 'stds_t': list(stds_t),
                 'means_f': list(means_f), 'stds_f': list(stds_f)})

    means_t = df_rtr.loc[(df_rtr['packetsize'] == s) & (df_rtr['hidden'] == False) & (
        df_rtr['rtscts'] == True)].sort_values(by=['datarate'], ascending=True)['mean']
    stds_t = df_rtr.loc[(df_rtr['packetsize'] == s) & (df_rtr['hidden'] == False) & (
        df_rtr['rtscts'] == True)].sort_values(by=['datarate'], ascending=True)['std']
    means_f = df_rtr.loc[(df_rtr['packetsize'] == s) & (df_rtr['hidden'] == False) & (
        df_rtr['rtscts'] == False)].sort_values(by=['datarate'], ascending=True)['mean']
    stds_f = df_rtr.loc[(df_rtr['packetsize'] == s) & (df_rtr['hidden'] == False) & (
        df_rtr['rtscts'] == False)].sort_values(by=['datarate'], ascending=True)['std']
    rtrs.append({'means_t': list(means_t), 'stds_t': list(stds_t),
                 'means_f': list(means_f), 'stds_f': list(stds_f)})
...
for i in range(len(thrs)):
    axes[i // 2][i % 2].set_title(('Limited range' if (i % 2) == 0 else 'Full range') +
                                  ' - Packet size: ' + str(packetsizes[i // 2]))
